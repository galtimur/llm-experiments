...
def get_tokens_from_vectors(
    embedding_matrix, batch_size, num_batches, do_rms=False, model_norm=None
):
    """
    ...
    vectors = torch.normal(
        mean=emb_mean, std=emb_std, size=(num_batches * batch_size, input_dim)
    )
...
def plot_emb_dist(emb):
    ...
    positions_to_plot = range(
        0, 4096, 10
    )  # Change step size depending on how dense you want the plot
